...
//turn around time -burst btime f
or(int i=0;i<n;i++)
{
wt[i]=tat[i]-bt[i];
}
...
{
int dup_at[n],dup_bt[n],tat[n],wt[n]; 
for (int i = 0 ; i < n ; i++)
dup_at[i] = at[i];
...
cout<< "\nProcesses "<< " Burst time "<<"\tArrival time "<<"turn around Time"<<"Complition Time"<<"waiting Time"<<"\n";
for (int i = 0 ; i < n ; i++)
cout<<processes[i]<<"\t\t "<<bt[i]<<"\t\t "<<at[i]<<"\t\t "<<tat[i]<<"\t\t "<<comp[i]<<"\t\t "<<wt[i]<<"\n";
}
//main code 
int main()
{
...
int processes[n]; int burst_time[n]; int arrival_time[n]; int priority[n];
for(int i=0;i<n;i++){
...
cout<<"Enter Priority of process no"<<i+1<<" :"; cin>>priority[i];
}
cout<<"input :- \n";
